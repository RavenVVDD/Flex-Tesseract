...
# Archivo para guardar las subregiones
subregiones_file = "subregiones.json"

# Cargar subregiones si existen
try:
    with open(subregiones_file, 'r') as file:
        subregiones_por_dia = json.load(file)
except FileNotFoundError:
    subregiones_por_dia = {"Lunes": [], "Martes": [], "Miércoles": [], "Jueves": [], "Viernes": []}

# Función para identificar el cordón, ciudad y subregión basada en el texto extraído
        ...
        for cordon, ciudades in cordones.items():
            for ciudad in ciudades:
                if ciudad in linea.upper():
                    subregion = lineas[idx + 1].strip() if idx + 1 < len(lineas) else ""
                    return (cordon, ciudad, subregion)  # Devuelve cordon, ciudad y subregion
...
def procesar_imagenes():
    ...
    if file_paths:
        for idx, file_path in enumerate(file_paths):
            ...
            # Obtener el cordón, ciudad y subregión detectada
            cordon, ciudad, subregion = identificar_cordon_por_ciudad(texto)
            ...
            if cordon == "cordon_no_identificado":
                ...
                # Si la subregión no se reconoce, permitir entrada manual
                if not subregion or subregion.strip() == "":
                    # Campo de entrada para subregión manual
                    entry_subregion = tk.Entry(frame)
                    entry_subregion.pack(side='left', padx=10)
                    entry_subregion.insert(0, "Ingresar subregión")

                    # Botón para confirmar subregión manual
                    btn_confirmar_subregion = tk.Button(frame, text="Confirmar Subregión", command=lambda e=entry_subregion, d=dia_actual, t=titulo, c=ciudad: agregar_subregion_manual(e, d, t, c))
                    btn_confirmar_subregion.pack(side='left', padx=10)
                else:
                    # Guardar la subregión en la lista para revisión manual
                    if dia_actual in subregiones_por_dia:
                        subregiones_por_dia[dia_actual].append({
                            "Etiqueta": titulo,
                            "Ciudad": ciudad if ciudad else "No identificada",
                            "Subregión": subregion if subregion else "No identificada"
                        })
        ...
        with open(subregiones_file, 'w') as file:
            json.dump(subregiones_por_dia, file, indent=4)
...
def agregar_subregion_manual(entry_subregion, dia_actual, titulo, ciudad):
    subregion_manual = entry_subregion.get()
    if subregion_manual.strip() != "":
        if dia_actual in subregiones_por_dia:
            subregiones_por_dia[dia_actual].append({
                "Etiqueta": titulo,
                "Ciudad": ciudad if ciudad else "No identificada",
                "Subregión": subregion_manual
            })
        # Guardar las subregiones en el archivo JSON
        with open(subregiones_file, 'w') as file:
            json.dump(subregiones_por_dia, file, indent=4)
        # Deshabilitar el campo de entrada y el botón
        entry_subregion.config(state='disabled')
    else:
        messagebox.showwarning("Advertencia", "Por favor, ingrese una subregión válida.")
...
def mostrar_tabla_semanal():
    ...
    total_semanal = 0  # Para calcular el total de la semana
    totals_por_cordon = {cordon: 0 for cordon in precios_cordon.keys()}  # Totales por cordón para la semana
    ...
    for row_num, (dia, data) in enumerate(datos_semanales.items(), start=1):
        ...
        for col_num, cordon in enumerate(headers[1:-2], start=1):
            ...
            totals_por_cordon[cordon] += cantidad  # Sumar al total semanal por cordón
    ...
    last_row = len(datos_semanales) + 1
    tk.Label(tabla_frame, text="Total Semana", font=('Arial', 12, 'bold')).grid(row=last_row, column=0, padx=10, pady=5)
    for col_num, cordon in enumerate(headers[1:-2], start=1):
        cantidad_total_cordon = totals_por_cordon[cordon]
        costo_total_cordon = precios_cordon.get(cordon, 0) * cantidad_total_cordon
        tk.Label(tabla_frame, text=f"{cantidad_total_cordon} (${costo_total_cordon})", font=('Arial', 12, 'bold')).grid(row=last_row, column=col_num, padx=10, pady=5)

    # Mostrar el total semanal en la última columna
    tk.Label(tabla_frame, text=f"${total_semanal}", font=('Arial', 12, 'bold')).grid(row=last_row, column=len(headers)-2, padx=10, pady=5)
...
def guardar_semana_excel():
    ...
    # Agregar totales semanales
    total_semanal = df['Total'].sum()
    totals_por_cordon = df[list(precios_cordon.keys())].sum()
    totals_por_cordon['Total'] = total_semanal
    totals_por_cordon.name = 'Total Semana'
    df = df.append(totals_por_cordon)
...
# Función para mostrar las subregiones capturadas
def mostrar_subregiones():
    ventana_subregiones = Toplevel(root)
    ventana_subregiones.title("Subregiones Capturadas")
    scrollbar = Scrollbar(ventana_subregiones)
    scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

    texto_area = Text(ventana_subregiones, wrap=tk.WORD, yscrollcommand=scrollbar.set)
    for dia, subregiones in subregiones_por_dia.items():
        if subregiones:
            texto_area.insert(tk.END, f"{dia}:\n")
            for entry in subregiones:
                etiqueta = entry.get("Etiqueta", "Etiqueta desconocida")
                ciudad = entry.get("Ciudad", "Ciudad no identificada")
                subregion = entry.get("Subregión", "Subregión no identificada")
                texto = f"{etiqueta} - {ciudad} ({subregion})\n"
                texto_area.insert(tk.END, texto)
...
# Botón para mostrar las subregiones capturadas
btn_mostrar_subregiones = tk.Button(root, text="Mostrar Subregiones", command=mostrar_subregiones)
btn_mostrar_subregiones.pack(pady=10)
